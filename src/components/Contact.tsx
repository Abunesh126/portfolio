...
const Contact: React.FC = () => {
  ...
  // Enhanced form submission with HloMail API
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      // Prepare data for HloMail API
      const formDataToSend = new FormData();
      formDataToSend.append("api_key", "78c4be0c6c8779406b83ded5172480a3");
      formDataToSend.append("name", formData.name.trim());
      formDataToSend.append("email", formData.email.trim());
      formDataToSend.append("message", formData.message.trim());
      formDataToSend.append("phone_no", formData.phone_no.trim());
      formDataToSend.append("template", "contact");

      // Send to HloMail API
      const response = await fetch("https://api.hlomail.in/v1/contact-mail", {
        method: "POST",
        body: formDataToSend,
      });
